...
class DetailPengajuanViewModel(private val apiInterface: ApiInterface) : ViewModel() {
    var idPemohon = 0
...
//    val listFile: LiveData<RealmResults<FileEntity>> get() = pkblRepository.getAllFile(idPemohon)
    ...
    fun postPenugasan(id: Int, koordinat: String) {
        _status = RemoteRepository(apiInterface)
            .postPenugasan(id, koordinat)
    }
    ...
    fun postPersetujuan(
        ...
        _status = RemoteRepository(apiInterface)
            .postPersetujuan(id, user, statusPersetujuan, alasan)
    }

    fun postPencairan(id: Int, user: String) {
        _status = RemoteRepository(apiInterface)
            .postPencairan(id, user)
    }
